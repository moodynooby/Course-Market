...
/**
 * Optimized time to minutes conversion with simple Map caching.
 * Removing manual LRU eviction as the space is bounded (1440 mins).
 */
export function timeToMinutesCached(time: string): number {
  let minutes = timeCache.get(time);
  if (minutes === undefined) {
    // Manual parsing is faster than .split().map(Number)
...
/**
 * Optimized retrieval of start and end minutes for a TimeSlot.
 * Uses WeakMap to cache results for the lifetime of the TimeSlot object.
 */
