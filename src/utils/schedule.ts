...
/**
 * Creates a ScoringContext to avoid redundant computations during batch schedule scoring.
 */
...
export function calculateScheduleScore(
  ...
  const excludeInstructorsSet = context?.excludeInstructorsSet ?? new Set(preferences.excludeInstructors);
  const daysUsed = new Set<DayOfWeek>();

  const preferredStart = context?.preferredStart ?? timeToMinutesCached(preferences.preferredStartTime);
  ...
  if (preferences.preferConsecutiveDays && daysUsed.size > 0) {
    const sortedDays = Array.from(daysUsed).sort(
      (a, b) => DAY_ORDER_MAP[a] - DAY_ORDER_MAP[b],
    );
