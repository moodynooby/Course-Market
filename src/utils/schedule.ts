...
/**
 * Converts a 24h time string (HH:MM) to total minutes from midnight.
 * Optimization: Uses a simple Map cache for the ~1440 possible values.
 * Faster manual parsing avoids .split() and .map() array allocations.
 */
export function timeToMinutesCached(time: string): number {
  ...
  const minutes =
    Number.parseInt(time.slice(0, colonIdx), 10) * 60 +
    Number.parseInt(time.slice(colonIdx + 1), 10);
...
/**
 * Gets the start and end minutes for a TimeSlot, with WeakMap caching.
 * Optimization: Avoids repetitive string lookups/hashing for TimeSlot objects.
 */
export function getSlotMinutes(slot: TimeSlot): { start: number; end: number } {
  const cached = slotMinutesCache.get(slot);
  if (cached) return cached;

  const start = timeToMinutesCached(slot.startTime);
  const end = timeToMinutesCached(slot.endTime);
  const result = { start, end };
  slotMinutesCache.set(slot, result);
  return result;
}
...
export function hasTimeConflict(slot1: TimeSlot, slot2: TimeSlot): boolean {
  ...
  const { start: start1, end: end1 } = getSlotMinutes(slot1);
  const { start: start2, end: end2 } = getSlotMinutes(slot2);
