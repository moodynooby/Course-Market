...
// 1440 minutes in a day, so we can cache all possible values if they are 1-min granular
const MAX_CACHE_SIZE = 1440;

/**
 * Converts "HH:mm" to minutes from midnight with caching.
 * Uses manual parsing for better performance than split().map(Number).
 */
...
/**
 * Returns pre-calculated minutes for a TimeSlot, or calculates and caches them.
 * This avoids redundant string parsing and Map lookups in tight loops.
 */
