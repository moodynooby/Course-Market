 ...
 * Domain is small (1440 mins/day), so we use a simple unbounded Map cache.
 * Parsing uses manual index lookups to avoid array allocation from .split().
 */
export function timeToMinutesCached(time: string): number {
  let minutes = timeCache.get(time);
  if (minutes === undefined) {
    const sep = time.indexOf(':');
    const hours = Number.parseInt(time.slice(0, sep), 10);
    const mins = Number.parseInt(time.slice(sep + 1), 10);
...
/**
 * Get pre-calculated start and end minutes for a TimeSlot.
 * Uses WeakMap to cache results by TimeSlot object identity, bypassing string lookups.
 */
