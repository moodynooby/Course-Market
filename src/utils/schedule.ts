...
export function formatTime(time24: string): string {
  const sepIdx = time24.indexOf(':');
  const hour = parseInt(time24.slice(0, sepIdx), 10);
  const minutes = time24.slice(sepIdx + 1);
...
export function timeToMinutesCached(time: string): number {
  ...
  if (minutes === undefined) {
    ...
    const hours = parseInt(time.slice(0, sepIdx), 10);
    const mins = parseInt(time.slice(sepIdx + 1), 10);
...
export function computeScheduleFeaturesWithContext(
  ...
  for (const section of sections) {
    for (const slot of section.timeSlots) {
      const dayIdx = DAY_TO_NUMBER[slot.day];
      daysMask |= 1 << dayIdx;
      if ((avoidDaysMask >> dayIdx) & 1) avoidDayHits++;
