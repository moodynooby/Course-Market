...
/** 1440 minutes in a day, covering all possible time strings */
const MAX_CACHE_SIZE = 1440;

/**
 * Converts HH:mm string to minutes since midnight with caching.
 * Uses manual parsing to avoid array allocations from split().
 */
...
/**
 * Retrieves pre-calculated minutes for a TimeSlot, using a WeakMap to
 * avoid redundant parsing and Map lookups in performance-critical paths.
 */
...
export function hasTimeConflict(slot1: TimeSlot, slot2: TimeSlot): boolean {
  ...
  const t1 = getSlotMinutes(slot1);
  const t2 = getSlotMinutes(slot2);
...
export function computeScheduleFeaturesWithContext(
  ...
  for (const section of sections) {
    for (const slot of section.timeSlots) {
      ...
      allSlots.push({ dayNum, start, end });
