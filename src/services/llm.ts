...
export async function generateScheduleAdvice(
  ...
  if (!isAvailable) {
    return 'LLM not available. Please configure a local LLM endpoint for AI-powered advice.';
  }

  const prompt = `As a course scheduling advisor, provide recommendations for the following courses:
${courses.join('\n')}

User Preferences:
- Preferred time: ${preferences.preferredStartTime} - ${preferences.preferredEndTime}
- Prefer morning: ${prefersMorning(preferences)}
- Prefer afternoon: ${prefersAfternoon(preferences)}
- Avoid days: ${preferences.avoidDays.join(', ') || 'None'}

Provide practical advice for selecting sections to create an optimal schedule.`;
  ...
  try {
    ...
    const data = await response.json();
    return data.message?.content || data.response || 'No advice generated.';
  } catch (error) {
    return `Failed to get AI advice: ${(error as Error).message}`;
  }
}

function prefersMorning(prefs: Preferences): string {
  return prefs.preferMorning ? 'Yes' : 'No';
}

function prefersAfternoon(prefs: Preferences): string {
  return prefs.preferAfternoon ? 'Yes' : 'No';
}
