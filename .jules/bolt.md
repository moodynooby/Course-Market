...
## 2026-05-21 - Structural Memoization with WeakMap
**Learning:** For performance-critical loops that repeatedly process stable objects (like TimeSlots in conflict detection or GeneratedSchedules in clustering), `WeakMap` based caching provides massive speedups by avoiding redundant parsing and feature computation. Manual string parsing for small, fixed domains (like "HH:MM") is significantly faster than `.split().map(Number)` because it avoids temporary array allocations and callback overhead.
**Action:** Use `WeakMap` to cache computed properties for stable objects in hot paths and prefer manual string parsing/indexing over higher-order array methods for micro-optimizations in tight loops.
