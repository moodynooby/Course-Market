...
**Learning:** In combinatorial search (schedule generation), the same TimeSlot objects are evaluated thousands of times. Caching parsed minutes on the object itself via WeakMap (or a property) eliminates O(N) string parsing overhead in the hottest loops. Bitmasks for DayOfWeek (0-6) provide a negligible but stable performance gain over Set lookups.
**Action:** Use WeakMap to cache derived numeric values on domain objects when they are used repeatedly in tight loops, and prefer bitmasks for small fixed domains like days of the week.
