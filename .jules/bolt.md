...
**Learning:** A simple Map is more efficient than a manual LRU cache for small, fixed domains like 24-hour time strings (1440 possible keys). WeakMap caching for complex objects like TimeSlot allows reusing expensive calculations (like numeric minute conversion) across multiple schedule evaluations without memory leaks.
**Action:** Replace manual LRU logic with simple Maps for small domains and use WeakMaps to cache pre-calculated properties on shared data objects in hot paths.
