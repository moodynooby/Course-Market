...
**Learning:** Finite domain string parsing (like HH:MM times) benefits more from a simple static Map cache than an LRU with eviction overhead. Using `WeakMap` to cache derived numeric values directly on data objects (like `TimeSlot`) eliminates redundant string operations and Map lookups in $O(N^2)$ paths like conflict detection.
**Action:** Use `WeakMap` for per-object derivation caching and prefer manual string slicing over `split().map(Number)` in high-frequency utility functions.
