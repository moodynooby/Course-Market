...
**Learning:** For small, finite key spaces like minutes-in-a-day (1440), an unbounded Map is faster and safer than a limited LRU cache that uses `array.shift()` (O(N)). WeakMap caching of parsed data on domain objects (`TimeSlot`) eliminates string hashing and Map lookup overhead in O(N^2) loops. Manual string parsing with `indexOf` and `slice` is ~2x faster than `split(':').map(Number)` due to reduced array allocations.
**Action:** Use unbounded Maps for finite domains and WeakMaps for object-level caching in performance-critical loops to minimize allocation and lookup overhead.
